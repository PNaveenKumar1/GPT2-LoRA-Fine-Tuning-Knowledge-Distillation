...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_out = self.aux_c_attn(x) + attn_out
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        # pass through the auxiliary LoRA layers
        x = self.aux_fc(x) + self.c_fc(x)
        x = self.gelu(x)
        x = self.aux_proj(x) + self.c_proj(x)
        x = self.dropout(x)
...
class LoRALinear(nn.Module):
    ...
    #     self.rank = rank
    #     self.dropout = nn.Dropout(0.1)
    #     self.L = nn.Parameter(torch.Tensor(in_features, rank))
    #     self.R = nn.Parameter(torch.Tensor(rank, out_features))
    #     # self.bias = nn.Parameter(torch.Tensor(out_features))
    #     self.reset_parameters()

    # def reset_parameters(self):
    #     nn.init.kaiming_uniform_(self.L)
    #     nn.init.kaiming_uniform_(self.R)
    ...
    #     return torch.matmul(torch.matmul(input, self.L), self.R)
    #     # lora_weights = torch.matmul(self.L, self.R)
    #     # return F.linear(input, lora_weights)
    # pass
